...
function pf_v11_student(array $u,int $sid): array { return pf_security_student(db(),$sid,$u); }
...
if($method==='GET'&&$route==='/goals'){
    $u=current_user();$sid=(int)($_GET['studentId']??0);if($u['role']==='student'){$s=pf_student_context($u);$sid=(int)$s['id'];}else{$s=pf_v11_student($u,$sid);}
    $q=$pdo->prepare('SELECT id,goal_type AS goalType,target_value AS targetValue,target_text AS targetText,unit,status,starts_at AS startsAt,due_at AS dueAt,completed_at AS completedAt,created_at AS createdAt FROM student_goals WHERE student_id=:sid ORDER BY status="active" DESC,id DESC');$q->execute(['sid'=>$sid]);json_response(['goals'=>$q->fetchAll()]);
}
if($method==='POST'&&$route==='/goals'){
    verify_csrf();$u=require_role('coach');$in=body();$sid=(int)($in['studentId']??0);pf_v11_student($u,$sid);$type=trim((string)($in['goalType']??''));if($type==='')json_response(['error'=>'Informe o tipo da meta.'],422);$q=$pdo->prepare('INSERT INTO student_goals(student_id,goal_type,target_value,target_text,unit,due_at) VALUES(:sid,:type,:value,:text,:unit,:due)');$q->execute(['sid'=>$sid,'type'=>$type,'value'=>isset($in['targetValue'])?(float)$in['targetValue']:null,'text'=>trim((string)($in['targetText']??'')),'unit'=>trim((string)($in['unit']??'')),'due'=>trim((string)($in['dueAt']??''))?:null]);$id=(int)$pdo->lastInsertId();audit($pdo,(int)$u['id'],'create','goal',$id,['studentId'=>$sid]);json_response(['ok'=>true,'id'=>$id],201);
}
...
if($method==='GET'&&$route==='/assessments'){
    $u=current_user();$sid=(int)($_GET['studentId']??0);if($u['role']==='student'){$s=pf_student_context($u);$sid=(int)$s['id'];}else pf_v11_student($u,$sid);
    $q=$pdo->prepare('SELECT id,weight,body_fat AS bodyFat,chest,waist,abdomen,hip,biceps_left AS bicepsLeft,biceps_right AS bicepsRight,thigh_left AS thighLeft,thigh_right AS thighRight,calf_left AS calfLeft,calf_right AS calfRight,notes,created_at AS createdAt FROM physical_assessments WHERE student_id=:sid ORDER BY id DESC');$q->execute(['sid'=>$sid]);json_response(['assessments'=>$q->fetchAll()]);
}
if($method==='POST'&&$route==='/assessments'){
    verify_csrf();$u=require_role('coach');$in=body();$sid=(int)($in['studentId']??0);pf_v11_student($u,$sid);$q=$pdo->prepare('INSERT INTO physical_assessments(student_id,coach_id,weight,body_fat,chest,waist,abdomen,hip,biceps_left,biceps_right,thigh_left,thigh_right,calf_left,calf_right,notes) VALUES(:sid,:coach,:weight,:bf,:chest,:waist,:abdomen,:hip,:bl,:br,:tl,:tr,:cl,:cr,:notes)');$q->execute(['sid'=>$sid,'coach'=>$u['id'],'weight'=>(float)($in['weight']??0),'bf'=>(float)($in['bodyFat']??0),'chest'=>(float)($in['chest']??0),'waist'=>(float)($in['waist']??0),'abdomen'=>(float)($in['abdomen']??0),'hip'=>(float)($in['hip']??0),'bl'=>(float)($in['bicepsLeft']??0),'br'=>(float)($in['bicepsRight']??0),'tl'=>(float)($in['thighLeft']??0),'tr'=>(float)($in['thighRight']??0),'cl'=>(float)($in['calfLeft']??0),'cr'=>(float)($in['calfRight']??0),'notes'=>trim((string)($in['notes']??''))]);$id=(int)$pdo->lastInsertId();$pdo->prepare('UPDATE students SET weight=:w,body_fat=:bf WHERE id=:id')->execute(['w'=>(float)($in['weight']??0),'bf'=>(float)($in['bodyFat']??0),'id'=>$sid]);audit($pdo,(int)$u['id'],'create','physical_assessment',$id,['studentId'=>$sid]);json_response(['ok'=>true,'id'=>$id],201);
}

if($method==='GET'&&$route==='/documents'){
    $u=current_user();$sid=(int)($_GET['studentId']??0);if($u['role']==='student'){$s=pf_student_context($u);$sid=(int)$s['id'];}else pf_v11_student($u,$sid);$q=$pdo->prepare('SELECT id,title,file_url AS fileUrl,document_type AS documentType,notes,created_at AS createdAt FROM student_documents WHERE student_id=:sid ORDER BY id DESC');$q->execute(['sid'=>$sid]);json_response(['documents'=>$q->fetchAll()]);
}
if($method==='POST'&&$route==='/documents'){
    verify_csrf();$u=require_role('coach');$in=body();$sid=(int)($in['studentId']??0);pf_v11_student($u,$sid);$title=trim((string)($in['title']??''));if($title==='')json_response(['error'=>'Informe o título do documento.'],422);$q=$pdo->prepare('INSERT INTO student_documents(student_id,coach_id,title,file_url,document_type,notes) VALUES(:sid,:coach,:title,:url,:type,:notes)');$q->execute(['sid'=>$sid,'coach'=>$u['id'],'title'=>$title,'url'=>trim((string)($in['fileUrl']??'')),'type'=>trim((string)($in['documentType']??'other')),'notes'=>trim((string)($in['notes']??''))]);json_response(['ok'=>true,'id'=>(int)$pdo->lastInsertId()],201);
}
