...
const ACCESS:Record<'admin'|'coach'|'student',ScreenView[]>={
 ...
 coach:['landing','coach_dashboard','prescription','student_detail','student_progress','anamnese','chat','agenda','assinatura'],
 student:['landing','student_hub','student_daily','workout_execution','student_progress','student_checkin','student_profile','anamnese','chat','agenda','assinatura']
};

export default function App(){
 const [currentScreen,setCurrentScreen]=useState<ScreenView>('landing');const [user,setUser]=useState<ApiUser|null>(null);const [booting,setBooting]=useState(true);const [selectedStudent,setSelectedStudent]=useState<Student>(EMPTY_STUDENT);const [isMetricsModalOpen,setIsMetricsModalOpen]=useState(false);const userRole:UserRole=user?.role||'public';
 useEffect(()=>{api.me().then(({user:sessionUser})=>{setUser(sessionUser);setCurrentScreen(HOME_BY_ROLE[sessionUser.role]);if(sessionUser.role==='coach')api.students().then(r=>{if(r.students?.[0])setSelectedStudent(normalizeStudent(r.students[0]) as Student)}).catch(()=>{})}).catch((e:unknown)=>{if(!(e instanceof ApiError)||e.status!==401)console.error(e);setUser(null)}).finally(()=>setBooting(false))},[]);
 ...
 const handleLoginSuccess=(authenticatedUser:ApiUser)=>{setUser(authenticatedUser);setCurrentScreen(HOME_BY_ROLE[authenticatedUser.role]);if(authenticatedUser.role==='coach')api.students().then(r=>{if(r.students?.[0])setSelectedStudent(normalizeStudent(r.students[0]) as Student)}).catch(()=>{})};
 const handleNavigate=(screen:ScreenView)=>{if(!allowedScreens.includes(screen)){setCurrentScreen(user?HOME_BY_ROLE[user.role]:'auth_login');return}setCurrentScreen(screen);window.scrollTo({top:0,behavior:'smooth'})};
 ...
 {currentScreen==='landing'&&<LandingScreen onNavigate={handleNavigate}/>} {currentScreen==='auth_login'&&<AuthLoginScreen onNavigate={handleNavigate} onLoginSuccess={handleLoginSuccess}/>} {user?.role==='coach'&&currentScreen==='coach_dashboard'&&<CoachDashboardScreen onNavigate={handleNavigate} onSelectStudent={handleSelectStudent}/>} {user?.role==='coach'&&currentScreen==='prescription'&&<PrescriptionScreen onNavigate={handleNavigate} selectedStudent={selectedStudent}/>} {(user?.role==='coach'||user?.role==='admin')&&currentScreen==='student_detail'&&<StudentDetailScreen student={selectedStudent} onNavigate={handleNavigate}/>} {user?.role==='student'&&currentScreen==='student_hub'&&<StudentHubScreen onNavigate={handleNavigate}/>} {user?.role==='student'&&currentScreen==='student_daily'&&<StudentDailyScreen onNavigate={handleNavigate}/>} {user?.role==='student'&&currentScreen==='workout_execution'&&<WorkoutExecutionScreen onNavigate={handleNavigate}/>} {(user?.role==='coach'||user?.role==='student')&&currentScreen==='student_progress'&&<StudentProgressScreen onNavigate={handleNavigate} onOpenMetricsModal={()=>setIsMetricsModalOpen(true)} selectedStudent={selectedStudent} userRole={userRole}/>} {user?.role==='student'&&currentScreen==='student_checkin'&&<StudentCheckinScreen onNavigate={handleNavigate}/>} {user?.role==='student'&&currentScreen==='student_profile'&&<StudentProfileScreen onNavigate={handleNavigate} onOpenMetricsModal={()=>setIsMetricsModalOpen(true)}/>} {(user?.role==='coach'||user?.role==='student')&&currentScreen==='anamnese'&&<AnamneseScreen onNavigate={handleNavigate} selectedStudent={selectedStudent} userRole={userRole}/>} {(user?.role==='coach'||user?.role==='student')&&currentScreen==='chat'&&<ChatScreen onNavigate={handleNavigate} selectedStudent={selectedStudent} userRole={userRole}/>} {(user?.role==='coach'||user?.role==='student')&&currentScreen==='agenda'&&<AgendaScreen onNavigate={handleNavigate}/>} {user&&currentScreen==='assinatura'&&<AssinaturaScreen onNavigate={handleNavigate} userRole={userRole}/>} {user?.role==='admin'&&currentScreen==='admin_system'&&<AdminDashboardScreen onNavigate={handleNavigate} onSelectStudent={handleSelectStudent}/>} </main></div>{!full&&user&&<MobileBottomNav currentScreen={currentScreen} onNavigate={handleNavigate} userRole={user.role}/>}<MetricsUpdateModal isOpen={isMetricsModalOpen} onClose={()=>setIsMetricsModalOpen(false)} onSave={handleSaveMetrics} initialData={{weight:selectedStudent.weight,bodyFat:selectedStudent.bodyFat}}/></div>;
}
