...
if($method==='PATCH'&&preg_match('#^/students/(\d+)/transfer$#',$route,$m)){
    ...
    $pdo->beginTransaction();try{$pdo->prepare('UPDATE students SET coach_id=:coach WHERE id=:id')->execute(['coach'=>$coach,'id'=>$id]);$pdo->prepare('UPDATE workouts SET coach_id=:coach WHERE student_id=:id')->execute(['coach'=>$coach,'id'=>$id]);$pdo->prepare('UPDATE appointments SET coach_id=:coach WHERE student_id=:id')->execute(['coach'=>$coach,'id'=>$id]);$pdo->prepare('UPDATE payments SET coach_id=:coach WHERE student_id=:id')->execute(['coach'=>$coach,'id'=>$id]);$pdo->prepare('UPDATE messages SET coach_id=:coach WHERE student_id=:id')->execute(['coach'=>$coach,'id'=>$id]);$pdo->commit();audit($pdo,(int)$actor['id'],'transfer','student',$id,'coach='.$coach);json_response(['ok'=>true]);}catch(Throwable $e){$pdo->rollBack();json_response(['error'=>'Não foi possível transferir o aluno.'],500);}
}
