...
function send_access_email(string $email,string $name,string $password,string $role):bool{$label=$role==='coach'?'treinador':($role==='admin'?'administrador':'aluno');return deliver_email($email,'Seu acesso ao PulseFit Pro',"Olá, {$name}!\n\nSua conta de {$label} no PulseFit Pro foi liberada.\n\nE-mail: {$email}\nSenha temporária: {$password}\n\nEntre no sistema e altere a senha no primeiro acesso.\n");}
