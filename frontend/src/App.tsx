...
export default function App(){
 ...
 useEffect(()=>{api.me().then(({user:sessionUser})=>{setUser(sessionUser);setCurrentScreen(HOME_BY_ROLE[sessionUser.role]);if(sessionUser.role==='coach'&&!sessionUser.mustChangePassword)api.students().then(r=>{if(r.students?.[0])setSelectedStudent(normalizeStudent(r.students[0]) as Student)}).catch(()=>{})}).catch((e:unknown)=>{if(!(e instanceof ApiError)||e.status!==401)console.error(e);setUser(null)}).finally(()=>setBooting(false))},[]);
