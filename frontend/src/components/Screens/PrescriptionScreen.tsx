  ...
  return <div className="w-full space-y-6">
    ...
    <div className="grid lg:grid-cols-12 gap-6">
      ...
      <aside className="lg:col-span-4 bg-[#121414] border border-[#333535] p-4 space-y-4 h-fit lg:sticky lg:top-20">
        <div className="flex justify-between items-center"><div><div className="text-[10px] font-mono text-[#DFFF00]">// BIBLIOTECA</div><h2 className="font-anybody font-bold uppercase text-white">Biblioteca de Exercícios</h2></div><button onClick={()=>setCustomOpen(v=>!v)} className="w-9 h-9 bg-[#DFFF00] text-black"><span className="material-symbols-outlined">add</span></button></div>
        ...
        <div className="flex gap-1">{filters.map(([id,label])=><button key={id} onClick={()=>setFilter(id)} className={`flex-1 py-1.5 text-[9px] font-mono border ${filter===id?'bg-[#DFFF00] text-black':'border-[#333] text-gray-400'}`}>{label}</button>)}</div>
        <div className="space-y-2 max-h-[580px] overflow-y-auto pr-1">{loading?<div className="text-xs font-mono text-gray-500">Carregando...</div>:filtered.map(x=><button key={x.id} onClick={()=>add(x)} className="w-full text-left flex gap-3 p-2 bg-[#181a1a] border border-[#333] hover:border-[#DFFF00] group"><div className="w-20 h-16 bg-black overflow-hidden shrink-0 border border-[#262828]"><ExerciseThumbnail name={x.name} category={x.category} equipment={x.equipment} src={x.thumbnail} mediaType={x.mediaType}/></div><div className="min-w-0 self-center"><div className="font-anybody text-xs font-bold uppercase text-white group-hover:text-[#DFFF00] truncate">{x.name}</div><div className="text-[9px] font-mono text-gray-500">{x.category} • {x.sets}x{x.reps}</div></div></button>)}</div>
      </aside>
    </div>
